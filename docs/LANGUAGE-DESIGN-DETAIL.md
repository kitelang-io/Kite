...
- **`String.length` = O(1) UTF-8 **byte** count** (equals `byteLen()`), which is what compiler spans need; scalar count is the explicit O(n) `chars().count()`. Pattern/closure examples use `s.length` for strings and `.size()` for collections (Kotlin naming).
